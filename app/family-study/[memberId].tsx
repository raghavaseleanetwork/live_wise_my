      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {profile.subjects.map((s) => (
          <View key={s.id} style={[styles.rowCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            {canEdit && (<Pressable onPress={() => removeSubject(s.id)} hitSlop={10}><Ionicons name="trash-outline" size={16} color={colors.textTertiary} /></Pressable>)}
          </View>
        ))}
        ...
        {profile.exams.map((e) => (
          <View key={e.id} style={[styles.rowCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            {canEdit && (<Pressable onPress={() => removeExam(e.id)} hitSlop={10}><Ionicons name="trash-outline" size={16} color={colors.textTertiary} /></Pressable>)}
          </View>
        ))}
        <View style={styles.addRow}>
          <TextInput
        ...
        {profile.events.map((ev) => (
          <View key={ev.id} style={[styles.rowCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            {canEdit && (<Pressable onPress={() => removeEvent(ev.id)} hitSlop={10}><Ionicons name="trash-outline" size={16} color={colors.textTertiary} /></Pressable>)}
          </View>
        ))}
        <View style={styles.addRow}>
          <TextInput
        ...
        {profile.fees.map((f) => (
          <View key={f.id} style={[styles.rowCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Pressable disabled={!canMarkDone} onPress={() => toggleFeePaid(f.id)} hitSlop={10}>
            ...
            {canEdit && (<Pressable onPress={() => removeFee(f.id)} hitSlop={10}><Ionicons name="trash-outline" size={16} color={colors.textTertiary} /></Pressable>)}
          </View>
        ))}
        <View style={styles.addRow}>
          <TextInput
